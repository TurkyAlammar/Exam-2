...
def add_exam(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            exam_type = form.data['exam_type']
            exam_time = form.data['exam_time']
            ...
            if temp:
                ...
                Exam.objects.get_or_create(examiner=request.user.myuser, exam_name=exam_name, category=category, exam_type=exam_type , exam_time=exam_time)
...
def edit_exam(request, exam_id):
    ...
    form.initial['exam_type'] = exam.exam_type
    form.initial['exam_time'] = exam.exam_time
...
def do_edit_exam(request, exam_id):
    ...
    if request.method == 'POST':
        ...
        if exam.exam_name == name and exam.category == category and exam.exam_type == exam_type and exam_time == exam_time:
            return redirect("main:all_exams")

        # check if found exam with same name and category
        exams = Exam.objects.filter(examiner=request.user.myuser).exclude(id=exam_id)
        temp = exams.filter(exam_name__iexact=name, category__iexact=category, exam_type__iexact=exam_type , exam_time__iexact=exam_time )
...
def show_exam(request, exam_id):
    ...
    path = MYDIR.removesuffix('main_app')
...
def do_exam(request, exam_id):
    ...
    if request.method == 'POST':
        ...
        print("Typeeeeeeeeeeee:",exam.exam_type)
        temp = ''
...
def update_question(request, exam_id, question_id):
    if request.method == 'POST':
        ...
        check = request.POST.get('check')
        print("Info == ", q_body, op1, op2, op3, op4, correct_ans, hint, check)
        if check == "1":
            image_file=''
        else:
            image_file= ""
